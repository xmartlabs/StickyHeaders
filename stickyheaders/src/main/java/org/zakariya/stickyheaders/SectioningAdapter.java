...
public class SectioningAdapter extends RecyclerView.Adapter<SectioningAdapter.ViewHolder> {
     ...
     * @param itemUserType if getSectionItemUserType is overridden to provide custom item types, this will be the type for this item
     */
    public void onBindItemViewHolder(ItemViewHolder viewHolder, int sectionIndex, int itemIndex, int itemUserType, @NonNull List<Object> payloads) {
        onBindItemViewHolder(viewHolder, sectionIndex, itemIndex, itemUserType);
    }
     ...
     * @param headerUserType if getSectionHeaderUserType is overridden to provide custom header types, this will be the type for this header
     */
    public void onBindHeaderViewHolder(HeaderViewHolder viewHolder, int sectionIndex, int headerUserType, @NonNull List<Object> payloads) {
        onBindHeaderViewHolder(viewHolder, sectionIndex, headerUserType);
    }
     ...
     * @param sectionIndex the index of the section containing the ghost header to update
     */
    public void onBindGhostHeaderViewHolder(GhostHeaderViewHolder viewHolder, int sectionIndex, @NonNull List<Object> payloads) {
        onBindGhostHeaderViewHolder(viewHolder, sectionIndex);
    }
     ...
     * @param footerUserType if getSectionFooterUserType is overridden to provide custom footer types, this will be the type for this footer
     */
    public void onBindFooterViewHolder(FooterViewHolder viewHolder, int sectionIndex, int footerUserType, @NonNull List<Object> payloads) {
        onBindFooterViewHolder(viewHolder, sectionIndex, footerUserType);
    }
    ...
    public void notifySectionItemChanged(int sectionIndex, int itemIndex, Object payload) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
        } else {
            ...
            notifyItemChanged(section.adapterPosition + itemIndex, payload);
        }
    }
    ...
    @Override
    public void onBindViewHolder(ViewHolder holder, int adapterPosition, @NonNull List<Object> payloads) {
        int section = getSectionForAdapterPosition(adapterPosition);
        ...
        switch (baseType) {
            case TYPE_HEADER:
                onBindHeaderViewHolder((HeaderViewHolder) holder, section, userType, payloads);
                break;
            ...
            case TYPE_ITEM:
                ...
                int positionInSection = getPositionOfItemInSection(section, adapterPosition);
                ivh.setPositionInSection(positionInSection);
                onBindItemViewHolder(ivh, section, positionInSection, userType, payloads);
                break;

            case TYPE_FOOTER:
                onBindFooterViewHolder((FooterViewHolder) holder, section, userType, payloads);
                break;

            case TYPE_GHOST_HEADER:
                onBindGhostHeaderViewHolder((GhostHeaderViewHolder) holder, section, payloads);
                break;
